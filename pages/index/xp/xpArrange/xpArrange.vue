<template>
	<view>
		...
		<view class="arrangeInfo" v-for="(item,index) in listInfo" :key="item.id">
			...
			<view class="mainBox" v-if="this.listInfo.id">内容</view>
			...
			<view class="button" v-else @click="updata(item.id)">安排选片档期</view>
...
<script>
	...
	import { getAllOrderItem, getOrdetItemListChoose } from '@/util/api/shop.js'
	export default {
		components:{
			detailMoudel,
		},
		...
		methods:{
			...
			updata(id){
				uni.navigateTo({
					url:'./AddorUpdata/AddorUpdata?itemNo=' + this.nextItemNo + '&name=' + this.nextName + '&type=' + this.type + '&itemId=' + this.itemId + '&id=' + id
				})
			},
		}
	}
