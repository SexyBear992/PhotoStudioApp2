	...
	<view class="bigBox">
		...
		<view v-for="item in dataInfo" :key="item.id">
			...
			<view class="listBox">
				...
				<view class="text">{{item.mobile | noData}}</view>
			</view>
			
			<!-- QQ -->
			<view class="listBox">
				<view class="title">QQ：</view>
				<view class="text">{{item.qq | noData}}</view>
			</view>
			
			<!-- 微信 -->
			<view class="listBox">
				<view class="title">微信：</view>
				<view class="text">{{item.wechat | noData}}</view>
			</view>
...
<script>
	export default{
		...
		data(){
			return{
				// 内容 relationCustomerContactVos
				dataInfo:[]
			}
		},
		watch:{
			openInfo(){
				this.dataInfo = this.openInfo.customerGroupVo.relationCustomerContactVos
			}
		}
	}
</script>
