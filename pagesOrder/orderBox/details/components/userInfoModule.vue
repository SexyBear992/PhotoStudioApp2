...
<template>
	<view class="bigBox">
		...
		<view class="listBox">
			...
			<view class="text">{{dataInfo}}</view>
		</view>
		
		<!-- 日期类型 -->
		<view class="listBox" v-if="typeTimeTitle">
			<view class="title">{{typeTimeTitle }}</view>
			<view class="text">{{openInfo.typeTime | time}}</view>
		</view>
...
<script>
	...
	export default{
		...
		watch:{
			openInfo(){
				let data = this.openInfo.customerGroupVo.categoryJson
				if(data.province){
					this.dataInfo = `${data.province}/${data.city}/${data.region}`
				}
